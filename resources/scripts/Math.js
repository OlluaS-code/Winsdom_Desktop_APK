          ...
          btn.addEventListener('click', () => {
              ...
              views.forEach(v => {
                  ...
                  if (v.id === `view-${targetView}`) {
                      v.style.display = targetView === 'calculator' ? 'flex' : 'block';
                  }
              });
              if (targetView === 'whiteboard' && window.app) window.app.resize();
          });
      });
  }
  ...
  function processMath() {
      ...
      try {
          ...
          if (inputVal.toLowerCase().startsWith('integral')) {
              let innerText = inputVal.match(/\((.+)\)/) ? inputVal.match(/\((.+)\)/)[1] : inputVal.replace(/integral/i, '').trim();
              ...
              plotExpression = node.type === 'FunctionAssignmentNode' ? node.expr.toString() : node.toString();
    ...
    function evaluateResult() {
        try {
            ...
            if (result === Infinity || isNaN(result)) {
                ...
                currentValue = Number.isInteger(result) ? result.toString() : parseFloat(result.toFixed(10)).toString();
